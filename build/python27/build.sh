...
LDFLAGS64="-L/usr/gnu/lib/sparcv9 -R/usr/gnu/lib/sparcv9"
CPPFLAGS="$CPPFLAGS -I/usr/include/ncurses -D_LARGEFILE64_SOURCE"
CPPFLAGS32="-I/usr/lib/libffi-$FFIVERS/include"
CPPFLAGS64="-I/usr/lib/sparcv9/libffi-$FFIVERS/include"
...
make_install32() {
    make_install
    rm $DESTDIR/usr/bin/python #|| logerr "--- cannot remove arch hardlink"
...
make_install64() {
    ...
    rm $DESTDIR/usr/bin/sparcv9/python #|| logerr "--- cannot remove arch hardlink"
    rm $DESTDIR/usr/lib/python2.7/config/libpython2.7.a || logerr "--- cannot remove static lib"
    (cd $DESTDIR/usr/bin/sparcv9 && ln -s python2.7 python) ||  logerr "--- could not setup python softlink"
    # Copy off _sysconfigdata.py
    sed 's/^/    /g' < $DESTDIR/usr/lib/python2.7/_sysconfigdata.py > \
