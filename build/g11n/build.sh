...
#    if [[ ! -d g11n ]]; then
#        logcmd  $GIT clone anon@src.omniti.com:~omnios/core/g11n
#    fi
    logcmd  cd g11n || logerr "g11n inaccessible"
#    logcmd $GIT checkout omni
...
install_man(){
    ...
    logcmd mkdir -p $SRC/proto/sparcv9/fileroot/usr/share/man/man5/ || \
        logerr "could not create destdir for man page"
    logcmd cp files/iconv_en_US.UTF-8.5 \
        $SRC/proto/sparcv9/fileroot/usr/share/man/man5/iconv_en_US.UTF-8.5 || \
        logerr "could not copy man page"
}
package(){
    pushd $TMPDIR/$BUILDDIR/g11n/pkg > /dev/null
    logmsg "--- packaging"
    ISALIST=sparcv9 CC=gcc logcmd $DMAKE \
	CLOSED_BUILD=no \
	L10N_BUILDNUM=$BUILDNUM \
	|| logerr "pkg make failed"
    ISALIST=sparcv9 CC=gcc logcmd $DMAKE publish_pkgs \
	SRC=$SRC \
	CLOSED_BUILD=no \
        L10N_BUILDNUM=$BUILDNUM \
