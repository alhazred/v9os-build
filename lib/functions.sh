...
ask_to_install() {
    ...
    if [[ -n "$AUTOINSTALL" ]]; then
        ...
        logcmd pkg install $PKG || logerr "pkg install $PKG failed"
        return
    fi
    if [[ -n "$BATCH" ]]; then
        logmsg "===== Build aborted ====="
    ...
    if [[ "$REPLY" == "i" || "$REPLY" == "I" ]]; then
        logcmd pkg install $PKG || logerr "pkg install failed"
    else
        logmsg "===== Build aborted ====="
        exit 1
    fi
...
PATH="/opt/gcc-5.1.0/bin:/opt/SUNWspro/bin:usr/ccs/bin:/usr/bin:/usr/sbin:/usr/gnu/bin:/usr/sfw/bin"
export PATH
...
EOF
    ...
    pushd ${DESTDIR}${LINTLIBDIR}/sparcv9 > /dev/null
