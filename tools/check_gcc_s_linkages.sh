...
GCC_S_64=`ldd /usr/lib/sparcv9/*.so 2>&1 | grep -v warning | \
    awk '/^\//{LIB=$1;} /libstdc/{LIB=""} /gcc_s/{if(LIB){print LIB;}}' | \
    grep -v gcc_s | cut -f1 -d:`

CPLUSPLUS=`ldd /usr/lib/{,sparcv9/}*.so 2>&1 | grep -v warning | \
...
for sym in `nm /usr/lib/sparcv9/libgcc_s.so | \
        sed -e 's/ *//g;' | \
        awk -F\| '{if($5=="GLOB" && $4=="FUNC" && $7!="UNDEF"){print $8;}}'`; do
    gcc64_s_sym+=([$sym]=1)
done
for sym in `nm /usr/lib/sparcv9/libc.so | \
        sed -e 's/ *//g;' | \
        awk -F\| '{if($5=="GLOB" && $4=="FUNC" && $7!="UNDEF"){print $8;}}'`; do
...
for so in /usr/lib/sparcv9/*.so; do
    [[ "$so" = "/usr/lib/sparcv9/libstdc++.so" ]] && continue
    syms=""
    for sym in `nm $so | sed -e 's/ *//g;' | \
            awk -F\| '{if($5=="GLOB" && $7=="UNDEF"){print $8;}}'`; do
